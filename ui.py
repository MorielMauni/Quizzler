...
class QuizInterface():
    ...
    def __init__(self, quiz_brain: QuizBrain):
        ...
        self.qa_text = self.canvas.create_text(150, 125, width=280, text="TEXT", fill=THEME_COLOR,font=("Arial", 20 , "italic"))
    ...
    def get_next_qa(self):
        self.canvas.config(bg="white")
        if self.quiz.still_has_questions():
            self.score_lable.config(text=f"Score: {self.quiz.score}")
            q_text = self.quiz.next_question()
            self.canvas.itemconfig(self.qa_text, text=q_text)
        else:
            self.canvas.itemconfig(self.qa_text, text="You have reached the end of the quiz")
    ...
    def give_feedback(self, is_right):
        ...
        self.window.after(1000, self.get_next_qa)
